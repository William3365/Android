...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"WiFI精控"对应的标识名
	protected static final String KEY_WIFI_ONOFF = "WiFI_onoff";
	// 数据点"沸水除氯"对应的标识名
	protected static final String KEY_CL2 = "cl2";
	// 数据点"恒温精控"对应的标识名
	protected static final String KEY_WENDU_CONTROL = "wendu_control";
	// 数据点"水温"对应的标识名
	protected static final String KEY_WENDU = "wendu";
	// 数据点"恒温精控"对应seekbar
	protected static final int WENDU_CONTROL_OFFSET = 0;
	// 数据点"恒温精控"对应数据点增量值
	protected static final int WENDU_CONTROL_ADDITION = 1;
	// 数据点"恒温精控"对应数据点定义的分辨率
	protected static final int WENDU_CONTROL_RATIO = 1;
	 ...
	 */
	// 数据点"WiFI精控"对应的存储数据
	protected static int data_WiFI_onoff;
	// 数据点"沸水除氯"对应的存储数据
	protected static int data_cl2;
	// 数据点"恒温精控"对应的存储数据
	protected static int data_wendu_control;
	// 数据点"水温"对应的存储数据
	protected static int data_wendu;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_WIFI_ONOFF)) {
					data_WiFI_onoff = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_CL2)) {
					data_cl2 = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_WENDU_CONTROL)) {
			
					data_wendu_control = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_WENDU)) {
			
					data_wendu = (Integer) map.get(dataKey);
				}
			}
		}
